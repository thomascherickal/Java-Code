 ...
 ********************************************************/    
 ...
 /**Inner class that implements a user interface
   ...
   /**sets up the JList lst
	...
	{
      lst = new JList();
      lst.setVisibleRowCount(MAX_SUBS);
      lst.setFixedCellWidth(200);
      String list_of_files[] = new String[MAX_SUBS];
	int i;
	for (i = 0; i < files[0].size(); i++)
      list_of_files[i] = (String)files[0].elementAt(i) + "              "; 
      ...
      public void actionPerformed(ActionEvent ae)
      {
         ...
         else if (ae.getSource() == combo)
         {
      	try {  
	       String list_of_files[] = new String[MAX_SUBS];
		   int i;
		   for (i = 0; i < files[Integer.parseInt(combo.getSelectedItem() + "")].size(); i++)
               list_of_files[i] = (String)files[Integer.parseInt(combo.getSelectedItem() + "")].elementAt(i); 
		   lst.setListData(list_of_files);
	...
	// add your own exceptions later to check for invalid arguments
     ...
     //check whether all arguments are supplied
      if (args.length == 0 || args.length == 1 || args.length == 2)
      {
	   ...
	   System.out.println("(1) packed array of number of files");
	   System.out.println("(2) starting value of client addresses"); 
	   System.out.println("(3)array of files as command line arguments");
	   System.out.println("eg. 11 5000 c1.java c2.java");
         System.exit(1);
      }
      else
      {
	   ...
	   int i, j, k = 2;
         char lengthch[] = args[0].toCharArray();
         int lengthint[] = new int[MAX_CLIENTS];
	   noclients = lengthch.length;

         for (i = 0; i < lengthch.length; i++)
         {    lengthint[i] = Integer.parseInt(""+ lengthch[i]);
		  addresses.addElement("" +  (Integer.parseInt(args[1])  + 10 * i));
	   }


         for (i = 0; i < lengthint.length; i++) {
		files[i] = new Vector(MAX_SUBS);
            for(j = 0; j < lengthint[i]; j++)
               files[i].addElement(args[k++]);
	    }
      }
}
/********************************************************
 ...
 ********************************************************/
  /**Main function
    *@param arg[0] contiguous string containing number of files of each client

...
class MultiProcessor implements Runnable
{
   ...
   synchronized void Execute(int k) throws InterruptedException
   {
      ...
      int l;
		...
		System.out.println(valueSet1);
	     	valueSet1 = (valueSet1 + 1) % n;
		notifyAll();

      
      while (i[k] < n / 2)
		{
			Butterfly(i[k], i[k] + (int)(n / Math.pow(2, j[k] + 1)), W[j[k]][i[k]]);
			finished[i[k]][k] = true;
    ...
    synchronized void Compute(int k) throws InterruptedException
    {
		...
		notifyAll();
    	   while (j[k] < (Math.log(n)/Math.log(2)))
	    {
	
           Execute(k);		
		
          stages[j[k]][k] = true;
            System.out.println("Thread " + k + "Stage " + j[k]);
      
           if(!CheckStageOver(j[k]))
             wait();
		else
		  notifyAll();  
		...
		System.out.println(valueSet3);

	     	valueSet3 = (valueSet3 + 1) % n;
		notifyAll();

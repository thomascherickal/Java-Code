...
{                     
   ...
   private class Parellel implements Runnable
   {
      ...
      {
         butterfly(a, bitRev[nt[a].j][a],nt[a].j,  nt[a].k, a);
         System.out.println("Butterfly " + bitRev[nt[a].j][a]);
      }            
       void butterfly(int a, int b, int j,int k, int c)
       {
            oldy[a] = y[a];
         if (a - b != k)  {
            y[a] = y[a].add((W[j][(int)(nt[c].i/2)].multiply(oldy[b])));
            System.out.println(y[a] + " add");
         }
         else  {
            y[b] = y[b].subtract((W[j][(int)(nt[c].i/2)].multiply(oldy[a])));
